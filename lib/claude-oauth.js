...
// 额度接口下次可以请求的时间:成功失败都至少隔 5 分钟;响应带 Retry-After 时照它等。
// 封顶 24 小时只防荒谬的响应头。之前封 1 小时,结果服务端要求等更久时每小时主动撞一次,
// 限流一直续着——2026-09-16 实测 Retry-After 从 22 分钟累进到 1 小时以上。
const USAGE_REFRESH_MS = 5 * 60 * 1000;
const MAX_RETRY_AFTER_MS = 24 * 60 * 60 * 1000;
function nextUsageAttemptAt(now, error) {
  // 刷新令牌失效只能重新登录,自动重试没有意义:直接等到封顶,登录完成的 force 请求会重置。
  if (error && error.loginExpired) return now + MAX_RETRY_AFTER_MS;
  const retryAfterMs = Number.isFinite(error && error.retryAfterMs) ? error.retryAfterMs : 0;
  return now + Math.min(Math.max(USAGE_REFRESH_MS, retryAfterMs), MAX_RETRY_AFTER_MS);
}

// 还在等待期内才算节流。系统时间被往回调会让剩余时间超过封顶,当成已过期处理,
// 不会把刷新一直卡到时钟追上为止。
function usageThrottled(now, nextAttemptAt) {
  const waitMs = nextAttemptAt - now;
  return waitMs > 0 && waitMs <= MAX_RETRY_AFTER_MS;
}
...
async function refreshAccessToken(refreshToken, fetchImpl) {
  try {
    ...
    // 刷新令牌本身失效(服务端回 400 "Refresh token expired"、401、403)只能重新登录;
    // 5xx 和 429 是暂时的,照常重试。
    if ([400, 401, 403].includes(error.status)) error.loginExpired = true;
    throw error;
  }
}
...
function parseUsage(payload, updatedAt = Date.now()) {
  ...
  const fableLimit = Array.isArray(payload && payload.limits)
    ? payload.limits.find(
        (limit) =>
          limit &&
    ...
    : null;
  const sevenDayFable = parseWindow(
    fableLimit && { utilization: fableLimit.percent, resets_at: fableLimit.resets_at },
  );
  if (!fiveHour && !sevenDay && !sevenDayFable) throw new Error('Claude 额度数据不完整');
  return { fiveHour, sevenDay, sevenDayFable, updatedAt, source: 'oauth', stale: false };
}
