...
test('floating UI crossfades states and labels inferred Desktop reset times', () => {
  ...
  assert.match(panel, /document\.documentElement\.addEventListener\('mouseleave'/);
...
test('identity detection returns stable hashes without exposing credentials', () => {
  ...
  fs.writeFileSync(codexPath, JSON.stringify({ OPENAI_API_KEY: 'codex-api-secret', auth_mode: 'api' }));
  const claude = detectClaudeIdentity(claudePath);
  const codex = detectCodexIdentity(codexPath);
...
test('usage worker shares cache and returns both provider ranges', () => {
  ...
  const cold = collectLocalUsage({ rangeDays: 10, now: NOW, caches, options });
  const warm = collectLocalUsage({ rangeDays: 10, now: NOW, caches, options });
  assert.equal(cold.claude.totals.input, 12);
  assert.equal(cold.codex.rangeDays, 10);
...
test('main and preload keep range and ledger operations narrowly scoped', () => {
  ...
  assert.match(preload, /setUsageRange: \(rangeDays\) => ipcRenderer\.invoke\('usage-range', rangeDays\)/);
