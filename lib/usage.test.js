...
test('Grok sessions aggregate per-turn usage with official cost and weekly quota', () => {
  ...
  const billing = (atMs, percent, currentPeriod) => JSON.stringify({
    ...
    ctx: { config: { creditUsagePercent: percent, currentPeriod }, subscriptionTier: 'X Premium' },
  });
  ...
  fs.writeFileSync(
    ...
    billing(NOW.getTime() - 6e4, 14, period(NOW.getTime() - 1000)) + '\n',
  );
  assert.equal(readQuota(path.join(root, 'logs', 'unified.jsonl'), NOW), null, 'expired period hides quota');
});
