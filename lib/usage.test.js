...
test('floating UI crossfades states and labels inferred Desktop reset times', () => {
  ...
  assert.match(html, /expanded\.position-right #surface \{ height: 476px; \}/);
...
test('Claude OAuth uses PKCE and parses official reset timestamps', async () => {
  ...
  assert.equal(nextUsageAttemptAt(0, { status: 429, retryAfterMs: 1e12 }), 3_600_000);
  ...
  // 用单调时钟,系统时间被往回调不能把刷新卡死。
  assert.match(main, /!force && performance\.now\(\) < claudeOAuthNextAttemptAt/);
  assert.match(main, /claudeOAuthNextAttemptAt = nextUsageAttemptAt\(performance\.now\(\)\);/);
  assert.match(main, /claudeOAuthNextAttemptAt = nextUsageAttemptAt\(performance\.now\(\), error\)/);
...
test('pricing handles per-model multipliers, vendor id shapes, and prefix collisions', () => {
  ...
  // Fable/Mythos 5.1 read cache at 0.025x input; every other model stays 0.1x.
  ...
  // Fast mode doubles Opus 5 / 4.8 only. Opus 4.6 accepts the flag but bills standard.
...
test('hang guard finds hung instances in tasklist output and flags stall/recovery once', () => {
  const { parseTasklistPids, hangTransition, HANG_AFTER_MS, CHECK_MS } = require('./hang-guard');
