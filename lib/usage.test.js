...
test('Claude Desktop metadata classifies transcripts and carries its title and bridge id', () => {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      bridgeSessionIds: ['junk', 'session_old', 'cse_latest'],
    }),
  );
  ...
  assert.equal(session.bridgeSessionId, 'cse_latest');
});

test('Desktop session links are validated and Claude falls back to copying its title', () => {
  assert.deepEqual(
  ...
  assert.deepEqual(
    ...
    { uri: 'claude://claude.ai/code/session_abc', exact: true },
  );
  assert.deepEqual(
    sessionTarget({ provider: 'claude', client: 'Desktop', title: 'A task title' }),
    { uri: 'claude://', exact: false, copyText: 'A task title' },
  );
  assert.throws(
    () => sessionTarget({ provider: 'codex', client: 'Desktop', sessionId: '../bad' }),
    /无效/,
  );
  assert.throws(() => sessionTarget({ provider: 'claude', client: 'CLI', title: 'x' }), /Desktop/);
});
...
test('Claude OAuth uses PKCE and parses official reset timestamps', async () => {
  ...
  assert.match(main, /fullscreenWatcher\.once\('error', resetFullscreenState\)/);
