...
// USD per 1M tokens — OpenAI standard API prices, see PRICE_SNAPSHOT.
// `npm run check-prices` diffs this table against LiteLLM's cost map.
// gpt-5.5-pro and gpt-5.5-cyber must stay listed even if Codex never selects them:
// without their own row they prefix-match gpt-5.5 and bill 6x / 2.5x under, silently.
//
// To add or change a model: one row here (USD per 1M tokens: input, cachedInput,
// output). Key = model id without date suffix; the longest matching key wins.
// Then set PRICE_SNAPSHOT above to today. Procedure: README, "How the cost is computed".
const PRICES = {
...
function priceFor(model) {
  ...
  // 官方文档写明 daybreak 两个 -latest 是别名,没有自己的价格行。
  if (model === 'gpt-5.6' || model === 'gpt-daybreak-blue-latest') return PRICES['gpt-5.6-sol'];
  if (model === 'gpt-daybreak-red-latest') return PRICES['gpt-5.6-cyber'];
