...
// WMI can be slow exactly when the machine is in the state that froze the app.
...
// execFile's error message only echoes the command back, which is exactly what the first
// real hang recorded and it explained nothing. Say whether it timed out, what it exited
// with, and what it printed.
function snapshotError(error, stderr) {
  if (!error) return null;
  const first = String(stderr || '').split(/\r?\n/).find(Boolean);
  const reason = error.killed
    ? `timed out after ${SNAPSHOT_TIMEOUT_MS} ms`
    : `exit ${error.code === undefined ? error.message : error.code}`;
  ...
  execFile(
    ...
    (error, stdout, stderr) => {
      ...
      append(logPath, {
        ...
        around: iso(lastBeat),
        processes,
        ...(error ? { error: snapshotError(error, stderr) } : {}),
      });
    },
  );
}
