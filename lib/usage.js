...
const PRICE_SNAPSHOT = '2026-09-06';
...
// USD per 1M tokens — Anthropic standard API list prices, see PRICE_SNAPSHOT.
// `npm run check-prices` diffs this table against LiteLLM's cost map.
...
// fast-mode multiplier (research preview, Opus 5 / 4.8 only — 4.7 rejects the
// request and 4.6 runs it at standard speed and standard price).
...
//
// To add or change a model: one row here. Key = model id WITHOUT the date
// suffix (priceFor matches by prefix; the longest key wins, so a longer sibling
// like claude-opus-4-5 next to claude-opus-4 needs its own row). Then set
// PRICE_SNAPSHOT above to today. Procedure: README, "How the cost is computed".
const PRICES = {
  ...
  'claude-opus-4-1': { input: 15, output: 75, legacy: true },
  'claude-opus-4': { input: 15, output: 75, legacy: true },
  'claude-sonnet-5': { input: 2, output: 10 },
  'claude-sonnet-4-6': { input: 3, output: 15 },
  'claude-sonnet-4-5': { input: 3, output: 15 },
  'claude-sonnet-4': { input: 3, output: 15, legacy: true },
  'claude-haiku-4-5': { input: 1, output: 5 },
  'claude-3-5-haiku': { input: 0.8, output: 4, legacy: true },
};
...
function scanFile(filePath, rangeStart) {
  let latestActivity = null;
  let customTitle = null;
  let firstPrompt = null;
  const messages = new Map();
  try {
    readJsonLinesSync(filePath, (line, lineNumber) => {
      let entry;
      try {
      ...
      const timestamp = new Date(entry.timestamp).getTime();
      if (!Number.isFinite(timestamp) || timestamp < rangeStart) return;
      // Streaming rewrites the same message id with growing usage — last wins.
      messages.set(msg.id || `${filePath}:${lineNumber}`, { timestamp, model: msg.model, usage: msg.usage });
    });
  } catch {
    return null;
  }
  return { activityAt: latestActivity, title: customTitle || firstPrompt, messages: [...messages.entries()] };
}

function cachedScan(filePath, stat, rangeStart, cache, diagnostics) {
  ...
  const summary = scanFile(filePath, rangeStart);
  if (diagnostics) {
...
function collectUsage({
  ...
  for (const transcriptRoot of transcriptRoots) {
    ...
    for (const dir of dirs) {
      ...
      for (const filePath of transcriptFiles(dirPath)) {
        ...
        const modifiedInRange = st.mtimeMs >= range.rangeStart;
        if (!recent && !modifiedInRange) continue;
        const transcript = cachedScan(filePath, st, range.rangeStart, cache, diagnostics) || {};
        for (const [messageId, message] of transcript.messages || []) seen.set(messageId, message);
        if (recent) {
  ...
  for (const { timestamp, model, usage } of seen.values()) {
    if (timestamp < range.rangeStart || timestamp > range.rangeEnd) continue;
    ...
    const costUSD = costOf(model, values);
    const add = (bucket) => {
      bucket.input += input;
    ...
    add((result.byModel[model] ||= { ...zero(), costUSD: 0 }));
    const daily = (result.daily[dayKey(new Date(timestamp))] ||= { byModel: {} });
    add((daily.byModel[model] ||= { ...zero(), costUSD: 0 }));
  }
